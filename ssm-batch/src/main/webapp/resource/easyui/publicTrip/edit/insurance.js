...
$(function(){
	var totalId = GetQueryString("totalId");
	$("#insurance").datagrid(
						{   width : "100%",
							height : "100%",
							checkbox : true,
							...
							method : 'get',
							url : 'publictripAction_editInsuranceCustList.do?totalId='+totalId,
							idField : 'id',
							columns : [[
							     		{field:'cust_name',title:'客户名称',width:"20%"},
							     		{field:'id',title:'id',hidden:true},
							     		{field:'cust_id',title:'客户编号',hidden:true},
							            {field : 'industry_classify',title : '行业分类',width :"15%",formatter:function(value,row,index){if(value=='3'){return "保险";}}},
						     		    {field:'business_scale',title:'业务规模',hidden:true},
									...
									onDblClickRow:function(){
										var arr = $('#insurance').datagrid('getSelections');
										$("#win").window("open");
										$("#win").panel({
											title : "查看信息"
										});
							        ...
							        onCheck:function(rowIndex, rowData){
							        	var arr = $('#insurance').datagrid('getSelections');
							        	$('#insuranceType').textbox('setValue',arr.length);
							        },
